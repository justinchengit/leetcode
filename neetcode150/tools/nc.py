...
""")
    ...
    for c in cats:
        ...
        s = len([p for p in group if is_solved(progress, p["slug"])])
        L.append(f"<details>\n<summary><b>{c}</b> &nbsp; {s}/{len(group)}</summary>\n")
        L.append("| | # | Problem | Difficulty | Solution | Conf | Last | Links |")
        L.append("|---|---|---|---|---|---|---|---|")
        for p in group:
            r = rec(progress, p["slug"])
            box = "x" if r.get("solved_on") else " "
            sol = ""
            for lang, ext in EXT.items():
                ...
                if f.exists():
                    ...
                    sol += f"[{ext}]({rel}) "
            L.append(
                f"| [{box}] | {p['id']} | {p['title']} | {p['difficulty']} | {sol.strip()} "
